...
let currentContent;
// @ts-ignore
const getSourceCode = (node: TSESTree.ConditionalExpression) => currentContent
    .getSourceCode(node)
    .getText(node);
...
export const astHelpersRN = {
    containsStyleSheetObject: function (
        ...
        return Boolean(
            ...
            && node.callee
            && 'object' in node.callee && node.callee.object
            && 'name' in node.callee.object && node.callee.object.name
    ...
    containsCreateCall: function (node: TSESTree.Node): boolean {
        return Boolean(
            node
            && 'callee' in node && node.callee
            && 'property' in node.callee && node.callee.property
    ...
    isStyleSheetDeclaration: function (node: TSESTree.Node, settings): boolean {
        ...
        return Boolean(
            astHelpersRN.containsStyleSheetObject(node, objectNames)
            && astHelpersRN.containsCreateCall(node)
        );
    },
            ...
            switch (node.type) {
                ...
                case AST_NODE_TYPES.TemplateLiteral:
                    return node.quasis.reduce((result, quasi, index) => result
                        + quasi.value.cooked
                        + astHelpersRN.getExpressionIdentifier(node.expressions[index]),
                        '');
                default:
                    return '';
            }
